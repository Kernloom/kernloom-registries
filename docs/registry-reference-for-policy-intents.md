...
|---|---|---|
| Policy intent | `kind: AccessPolicy` | Subject/resource types, context keys in conditions, risk/posture values, constraints |
...
| Entity Taxonomy | `registries/taxonomy/entities.yaml` | Base vocabulary for entity, subject, resource, and environment types; network zones; fact status; support/fidelity; and delegation status. | Used for canonical resource/environment types and concrete subject identities. Forge selectors such as `role` or `group` are policy selectors and must map to canonical subjects such as `user`, `service_account`, `workload`, or `external_partner`. |
...
Here, `user` and `application_group` come from the Entity Taxonomy. If the
policy uses `role` or `group` as a subject selector instead, Forge must map it
through IdP/CMDB context to canonical subjects. The three `signal` values come
from Context Keys. `low` and the risk quality rules come from the Risk Taxonomy.
Forge checks the constraints against support, gap, and granularity rules.
